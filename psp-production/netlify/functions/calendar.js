...
function makeJWT() {
  ...
  key = key.replace(/\\n/g, "\n");
  // If the key doesn't have proper line breaks in the body, add them
  if (!key.includes("\n-----END")) {
    key = key
      .replace("-----BEGIN PRIVATE KEY-----", "-----BEGIN PRIVATE KEY-----\n")
      .replace("-----END PRIVATE KEY-----", "\n-----END PRIVATE KEY-----");
    // Add line breaks every 64 chars in the base64 body
    const lines = key.split("\n");
    const header64 = lines[0];
    const footer64 = lines[lines.length - 1];
